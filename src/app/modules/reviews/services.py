...
class ReviewService:
    ...
    def create_review(self, card_id: int, rating: int):
        ...
        if rating < 1 or rating > 5:
            logger.warning("create_review validation failed: rating out of range (1-5): %s for card_id=%s", rating, card_id)
            raise ValueError("Rating must be 1–5")

        try:
            review = self.repo.create(card_id, rating)
            logger.info("Review created: card_id=%s rating=%s review_id=%s", card_id, rating, getattr(review, "id", None))
            return review
        except Exception:
    ...
    def compute_deck_score(self, deck_id: int):
        ...
        count = len(scores)
        logger.info("Computed deck score: deck_id=%s average_score=%s review_count=%s", deck_id, average, count)
    ...
    def delete_deck_reviews(self, deck_id: int):
        ...
        try:
            ...
            logger.info("Deleted reviews for deck_id=%s result=%s", deck_id, getattr(result, "deleted_count", result))
            return result
        except Exception:
